...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <aside className={cn("fixed inset-y-0 left-0 z-40 hidden border-r border-slate-100 bg-white shadow-[1px_0_2px_rgba(15,23,42,0.02)] transition-[width] duration-200 dark:border-border dark:bg-card lg:block", sidebarCollapsed ? "w-[72px]" : "w-64")}>
      ...
      <div className={cn("transition-[padding] duration-200", sidebarCollapsed ? "lg:pl-[72px]" : "lg:pl-64")}>
        ...
        <main className="p-4 md:p-6 lg:p-8">
          <nav className="mx-auto mb-5 flex max-w-[1440px] items-center gap-1.5 text-xs text-[#6F7882]" aria-label="Breadcrumb">
