...
type DefinitionInput = Omit<SystemSettingDefinition, "group">;

const option = (value: string, label = value): SystemSettingOption => ({ value, label });
const field = (group: SystemSettingGroup, input: DefinitionInput): SystemSettingDefinition => ({ group, ...input });

const general: SystemSettingDefinition[] = [
...
const localization: SystemSettingDefinition[] = [
...
const accounts: SystemSettingDefinition[] = [
...
const security: SystemSettingDefinition[] = [
...
const storage: SystemSettingDefinition[] = [
  ...[
    ["avatar", "Avatar", 5],
    ...
    ["banner", "Banner", 15],
  ].map(([key, label, defaultValue]) => field("storage", { key: `upload.maxMb.${key}`, label: `Dung lượng tối đa: ${label} (MB)`, priority: "P0", type: "number", defaultValue: defaultValue as number, min: 1, max: 100, integer: true })),
...
const scholarships: SystemSettingDefinition[] = [
...
const applications: SystemSettingDefinition[] = [
...
const kyc: SystemSettingDefinition[] = [
...
const support: SystemSettingDefinition[] = [
...
const notifications: SystemSettingDefinition[] = [
...
const seo: SystemSettingDefinition[] = [
...
const DEFINITION_BY_KEY = new Map(SYSTEM_SETTING_DEFINITIONS.map((definition) => [definition.key, definition]));

export function getSystemSettingDefinition(key: string) {
  return DEFINITION_BY_KEY.get(key);
}

export function getSystemSettingDefinitions(group?: SystemSettingGroup) {
  return group ? SYSTEM_SETTING_DEFINITIONS.filter((definition) => definition.group === group) : [...SYSTEM_SETTING_DEFINITIONS];
}

export function isSystemSettingGroup(value: string): value is SystemSettingGroup {
  return SYSTEM_SETTING_GROUPS.some((group) => group.id === value);
}

export function validateSystemSettingValue(definition: SystemSettingDefinition, value: unknown): string | null {
  if (definition.type === "boolean") {
    return typeof value === "boolean" ? null : "phải là true hoặc false";
  }
  if (definition.type === "number") {
    if (typeof value !== "number" || !Number.isFinite(value)) return "phải là số hợp lệ";
    if (definition.integer && !Number.isInteger(value)) return "phải là số nguyên";
    if (definition.min !== undefined && value < definition.min) return `phải lớn hơn hoặc bằng ${definition.min}`;
    if (definition.max !== undefined && value > definition.max) return `phải nhỏ hơn hoặc bằng ${definition.max}`;
    return null;
  }
  if (definition.type === "list") {
    if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) return "phải là danh sách chuỗi";
    if (value.length > 500) return "không được vượt quá 500 phần tử";
    return null;
  }
  if (definition.type === "json") {
    if (value === null || typeof value !== "object") return "phải là JSON object hoặc array";
    return null;
  }
  if (typeof value !== "string") return "phải là chuỗi";
  if (value.length > 100_000) return "quá dài";
  if (definition.type === "email" && value && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) return "không phải email hợp lệ";
  if (definition.type === "url" && value) {
    try {
      const parsed = new URL(value);
      if (!["http:", "https:"].includes(parsed.protocol)) return "chỉ chấp nhận URL http/https";
    } catch {
      return "không phải URL hợp lệ";
    }
  }
  if (definition.type === "color" && !/^#[0-9A-Fa-f]{6}$/.test(value)) return "phải là mã màu hex dạng #0866FF";
  if (definition.type === "select" && definition.options && !definition.options.some((item) => item.value === value)) return "không thuộc danh sách cho phép";
  return null;
}

export function getSystemSettingDefaults(group?: SystemSettingGroup): Record<string, SystemSettingValue> {
  return Object.fromEntries(getSystemSettingDefinitions(group).map((definition) => [definition.key, definition.defaultValue]));
}

export const SENSITIVE_SYSTEM_SETTING_KEYS = SYSTEM_SETTING_DEFINITIONS
